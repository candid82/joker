...
Note that the `-tags fast_init` (fast-startup) build of Joker maps all the core namespaces at build time; all compiled-in namespaces therefore start out mapped, not merely available.
...
As this approach does *not* involve the normal Read phase at Joker startup time, the overhead involved in parsing some Clojure forms is avoided, in lieu of using (what one assumes would be) faster code paths that convert binary blobs directly to AST forms. However, most types of objects are still encoded as text (albeit somewhat "squished"), rather than in binary, so there's still substantial overhead in reading, parsing, and reconstructing those objects from the text blobs.

Even _mapping_ of a core namespace does not occur until it is needed, though this likely saves very little time and somewhat complicates the code. See [Map All Builtin (Core) Namespaces?](https://github.com/candid82/joker/issues/326) for a discussion of this issue.

A disadvantage of this approach is that it requires changes to `core/pack.go` when changes are made to the binary-encoded AST forms (`Object`s).
...
Note that core libraries (other than `joker.core` and, when running the Repl, `joker.repl`) do not, in the slow-startup version of Joker, show up in `(all-ns)` until after they've been loaded via `:require` or similar.
