...
* A new `core/gen_code/gen_code.go` program parallels (and is run in addition to) `gen_data.go`. It generates `a_*code.go` files that mostly define static variables representing the structures resulting when loading `core/data/core.joke` and the like; these are `// +build fast_init`, so they are compiled only when building the fast-startup version of Joker.
