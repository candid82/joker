...
Day 11 still returns `31`: VM **22.06–22.55s**, ~**13.53GB** sampled allocations and **332–334MB** peak RSS, versus `--no-vm` **31.54–31.86s**, **326–327MB** peak RSS. Broader compilation and tracking source positions cost roughly 1–1.5 seconds of VM wall time on this workload compared with the narrow build, but remove policy-based restrictions independent of this particular benchmark. The remaining ~0.99M VM→AST calls target the workload's `near-floors` (line 27), called from `next-states` (line 42).
